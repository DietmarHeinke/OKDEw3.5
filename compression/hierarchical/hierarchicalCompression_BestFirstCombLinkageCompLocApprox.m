function [pdf_cmprs , idxToref_out, augmented_pdf ]= hierarchicalCompression_BestFirstCombLinkageCompLocApprox( pdf, varargin )
...
for i = 1:2:nargs
    switch args{i}
        ...
        case 'original_pdf', original_pdf = args{i+1} ;
...
assign_clust_type = 'centroid'; % median 'complete' ;% centroid, single, weighted, average, ward ;
type_dist_calculation = 'eucledian' ; %'eucledian' ; 'mahalanobis'
% calculate linkage and clustering
switch type_dist_calculation
    case 'eucledian'
        centers = pdf.Mu ;
        %centers = centers ./ repmat(std(centers')', 1, size(centers,2)) ;
		
		centers = bsxfun(@times, centers, 1./std(centers')') ;
		
%		centers = centers ./ abs( bsxfun(@minus, list_of_visited_solutions, rsp) ) ;	
		
        Y = pdist(centers','euclidean') ;  % was 'seuclidean'  
    case 'mahalanobis'
...
%     end
% *length(otherClasses.pdf.pdfs) *length(otherClasses.pdf.pdfs)
% TA JE PRAVI!!!!!!!
    if sqrt(sum(cmp_data.hells.*cmp_data.w))*otherClasses.pdf.N_all_classes <= costThreshold && minNumberOfComponentsThreshold <= length(cmp_data.idxToref)
       break ; %*length(otherClasses.pdf.pdfs)
    end

%     if  (sum(cmp_data.hells.*cmp_data.w))*otherClasses.pdf.N_all_classes <= costThreshold && minNumberOfComponentsThreshold <= length(cmp_data.idxToref)
%        break ; %*length(otherClasses.pdf.pdfs)
%     end
...
% pdf_prop = pdf ; % MODIFIED!!!!
            ...
            H_tmp(i) = uCostModel( otherClasses.pdf, pdf_ref, pdf_compr,  otherClasses.priors, approximateCost, pdf_prop, type_cost ) ;              
       end
    end
    ...
    if isnan(sum(cmp_data.hells)) || isinf(sum(cmp_data.hells)) 
        error('One of the Hellinger distances in Inf or NaN!') ;
    end
end
  
if ~isempty(original_pdf)
    pdf = original_pdf ;
end
...
% if isempty(otherClasses)
    for i = 1 : num_of_comps
        ...
        [ pdf_out, new_Cov, new_mu, w_out ] = ...
            combineSubLayersOf( pdf.smod.q(id_sel), pdf.w(id_sel),...
            pdf.smod.H, type_dist_calculation,...
            assign_clust_type, turn_off_splitting ) ;
        catch
            sf
        end
        q = horzcat(q, pdf_out) ;
...
% ----------------------------------------------------------------------- %
function [ pdf_out, new_Cov, new_mu, w_out ] = combineSubLayersOf( q_set, w, H, ...
                                    type_dist_calculation, assign_clust_type,...
                                    turn_off_splitting) %pdf, idx_src_cmps ) 
...
if turn_off_splitting == 0
    ...
    [new_mu, new_Cov, w_out] = momentMatchPdf(q_pdf.Mu, q_pdf.Cov, q_pdf.w) ;
    pdf_out.Mu = new_mu ;
    pdf_out.Cov = {new_Cov} ;
    pdf_out.w = w_out ;
end
...
function [nodes, ref_idx] = getSubclustersOfNodeIndex( clustering, id_node_name )
 if id_node_name <= clustering.len_data
     nodes = id_node_name ;
     ref_idx = {[id_node_name]} ;
 end
