function h=plotgauss2d(mu, Sigma, color, thickness)
...
h = plotcov2(mu, Sigma,color,thickness);
return;
...
function h = plotcov2(mu, Sigma, color, thickness, varargin)
...
[p, ...
 n, ...
 plot_opts] = process_options(varargin, 'conf', 0.682, ... %0.9, ...
					'num-pts', 100);
h = [];
holding = ishold;
if (Sigma == zeros(2, 2))
...
h = [h; plot(z(1, :), z(2, :), 'color', color, 'LineWidth', thickness)];
